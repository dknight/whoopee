...
create_html_page() {
    ...
    # html, head
    {
        ...
        if [[ ${filename%.*} != $index_file ]]; then
            echo -n "<a href=\"$index_file\">&larr; $template_archive_index_page</a>"
        fi
        ...
        # one blog entry
        if [[ $index == no ]]; then
            echo -n "<!-- entry begin -->" # marks the beginning of the whole post
            echo -n "<a name=\"begin\"></a>"
            echo "<h1><a class=\"ablack\" href=\"#begin\">"
            ...
            echo "</a></h1>"
            if [[ -z $timestamp ]]; then
                echo -n "<!-- $date_inpost: #$(LC_ALL=$date_locale date +"$date_format_timestamp")# -->"
            else
                echo -n "<!-- $date_inpost: #$(LC_ALL=$date_locale date +"$date_format_timestamp" --date="$timestamp")# -->"
            fi
            if [[ -z $timestamp ]]; then
                echo -n "<div class=\"subtitle\"><time pubdate=\"$(LC_ALL=$date_locale date +"$pubdate_format")\">$(LC_ALL=$date_locale date +"$postdate_format")"
            else
                echo -n "<div class=\"subtitle\"><time pubdate=\"$(LC_ALL=$date_locale date +"$pubdate_format")\">$(LC_ALL=$date_locale date +"$postdate_format" --date="$timestamp")"
            fi
            echo -n "</time>"
            # don't print author for post
            #[[ -n $author ]] && echo -e " &mdash; \n$author"
            echo -n "</div>"
            echo -n "<!-- text begin -->" # This marks the text body, after the title, date...
        fi
        ...
        if [[ $index == no ]]; then
            ...
            echo -n "<!-- entry end -->" # absolute end of the post
        fi
        ...
        #if [[ ${filename%.*} != $archive_index ]]; then
            #echo -n "<a href=\"$archive_index\">$template_archive</a>"
        #fi
        #if [[ ${filename%.*} != $tags_index ]]; then
            #echo -n "<a href=\"$tags_index\">$template_tags_title</a>"
        #fi
        #echo -n "</nav>"
        echo -n "</main>"
...
{
    echo "<h1>$template_archive_title</h1>"
    prev_month=""
    while IFS='' read -r i; do
        ...
        if [[ $month != "$prev_month" ]]; then
            [[ -n $prev_month ]] && echo -n "</ul>"  # Don't close ul before first header
            echo "<h4>$month</h4>"
        ...
        pubdate=$(LC_ALL=$date_locale date -r "$i" +"$pubdate_format")
        echo "<time pubdate=\"$pubdate\">$date</time>"
        echo -n "<a href=\"$i\">$title</a>"
        echo "</li>"
    done < <(ls -t ./*.html)
    echo "" 1>&3
    echo -n "</ul>"
...
{
    echo "<h1>$template_tags_title</h1>"
    echo "<ul id=\"articles-list\">"
    for i in $prefix_tags*.html; do
        [[ -f "$i" ]] || break
        echo -n "." 1>&3
        #nposts=$(grep -c "<\!-- text begin -->" "$i")
        # One <li> is determined for posts count
        nposts=$(grep -c "<li>" "$i")
        ...
        echo "<li><a href=\"$i\">$tagname</a> <span class=\"posts-count\">$nposts $word</span></li>"
    done
    echo "" 1>&3
    echo -n "</ul>"
} 3>&1 > "$contentfile"
...
while IFS='' read -r i; do
    ...
    pubdate=$(LC_ALL=$date_locale date -r "$i" +"$pubdate_format")
    echo -n "<li>"
    echo -n "<time pubdate=\"$pubdate\">$date</time> " # space at end
        echo -n "<a href=\"$i\">$title</a>"
        echo -n "</li>"
        #fi
        echo -n "." 1>&3
...
set -- "${@/#/$prefix_tags}"
set -- "${@/%/.html}"
sed -n '/^<h1><a class="ablack" href="[^"]*">/{s/.*href="\([^"]*\)">.*/\1/;p;}' "$@" 2> /dev/null
}
...
while IFS='' read -r i; do
    ...
    echo -en "<ul id=\"articles-list\">$(cat $i)</ul>" > "$i"
    echo -en "<h1>Tag: $tagname</h1>$(cat $i)" > "$i"
...
# $1 the html file
get_post_title() {
    awk '/<h1><a class="ablack" href=".+">/, /<\/a><\/h1>/{if (!/<h1><a class="ablack" href=".+">/ && !/<\/a><\/h1>/) print}' "$1"
}
...
create_includes() {
    {
        echo -n "<img src=\"/assets/gnu-zoo.png\" alt=\"GNU and Tux\">"
        echo -n "<h3 id=\"blog-title\">$global_description</h3>"
    } > ".title.html"
    ...
    if [[ -f $header_file ]]; then cp "$header_file" .header.html
        ...
        echo -n "<meta name=\"author\" content=\"$global_author\">"
        echo -n "<meta name=\"description\" content=\"$global_description\">"
        echo -n "<meta name=\"version\" content=\"$(cat VERSION)\">"
        echo -n "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">"
        printf "<link rel=\"stylesheet\" href=\"%s?$(cat VERSION)\" type=\"text/css\">" "${css_include[@]}"
        if [[ -z $global_feedburner ]]; then
            echo -n "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"$template_subscribe_browser_button\" href=\"${global_url}${blog_feed}\">"
        else 
            echo -n "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"$template_subscribe_browser_button\" href=\"$global_feedburner\">"
        fi
        } > ".header.html"
    fi
    ...
    if [[ -f $footer_file ]]; then cp "$footer_file" .footer.html
        ...
        echo -n "<footer role=\"contentinfo\">"
        echo -n "Copyright &copy; 2018 $global_author &mdash; <a href=\"mailto:$protected_mail\">$protected_mail</a>"
        #echo -n "<br>Verbatim copying and redistribution of this entire page are permitted provided this notice is preserved. <a href=\"https://creativecommons.org/licenses/by-nd/3.0/\">$global_license</a>"
        echo -n "&nbsp;"
        echo -n "<a href=\"$blog_feed\">"
        echo -n "<img src=\"/assets/feed.svg\" width=\"16\" alt=\"$template_subscribe\"></a>"
        echo -n "</footer>"
        } >> ".footer.html"
    fi
}
